...
- Express (v4.18.2): A web framework for Node.js that simplifies the creation of web applications.
- Langdetect (v0.2.1): A language detection library for Node.js, used to identify the language of text content.
- Lodash (v4.17.21): A utility library delivering various tools to manage Javascript code.
- Nodemon (v3.0.3): A tool that helps develop Node.js applications by automatically restarting the server when file changes are detected.
- Puppeteer (v21.11.0): A Node library that provides a high-level API over the headless Chrome browser, allowing you to automate tasks on websites, such as opening pages, clicking buttons, and extracting information.
